...
export const ThemeApplier = ({ theme }: { theme: any }) => {
  useEffect(() => {
    document.documentElement.style.setProperty('--bg-app', theme.isLight ? '#f8fafc' : (theme.name === 'AMOLED Black' ? '#000000' : '#060413'));
    document.documentElement.style.setProperty('--bg-panel', theme.isLight ? '#ffffff' : (theme.name === 'AMOLED Black' ? '#0a0a0a' : 'rgba(15, 23, 42, 0.45)'));
    ...
    style.innerHTML = `
      ...
      .bg-slate-900, .bg-slate-900\\/40, .bg-slate-950, .bg-slate-950\\/95, .bg-slate-900\\/80 { background-color: var(--bg-panel) !important; }
    ...
    return () => { const e = document.getElementById('theme-overrides'); if(e) e.remove(); }
  }, [theme]);
  return null;
};
